...
RSpec.describe Review, type: :model do
  ...
  describe 'Model Testing' do
    it '.sort_reviews_by_created_at(:desc)' do
      expect(Review.sort_reviews_by_created_at(:desc).last.title).to eq("S'okay")
      expect(Review.sort_reviews_by_created_at(:desc)[1].title).to eq("Great Book, Lousy References")
      expect(Review.sort_reviews_by_created_at(:desc).first.title).to eq("Insightful, practical, and colorful. Perspective from a biased reviewer.")
    end

    it 'sort_reviews_by_created_at(:asc)' do
      expect(Review.sort_reviews_by_created_at(:asc).first.title).to eq("S'okay")
      expect(Review.sort_reviews_by_created_at(:asc)[1].title).to eq("Great Book, Lousy References")
      expect(Review.sort_reviews_by_created_at(:asc).last.title).to eq("Insightful, practical, and colorful. Perspective from a biased reviewer.")
    end
